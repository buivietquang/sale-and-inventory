    ...
    public partial class frmAddProduct : Form
    {
       ...
       // string connection = @"Provider=Microsoft.ACE.OLEDB.12.0;Data Source=|DataDirectory|\Data.accdb";
        ...
        public void getManufacturer()
        {
            try
            {
                // Câu lệnh SQL để lấy tất cả bản ghi từ bảng tblManufacturer
                string sql2 = @"Select * from tblManufacturer";
                cm = new SqlCommand(sql2, cn);
                // Thực thi câu lệnh SQL và lấy kết quả vào DataReader
                dr = cm.ExecuteReader();
                // Duyệt qua từng bản ghi trong DataReader
                while (dr.Read())
                {
                    // Thêm dữ liệu từ cột thứ hai của mỗi bản ghi vào comboBox cboManufac
        ...
        private void Form11_Load(object sender, EventArgs e)
        {
            ...
            getData();
            // Tạo ID mới cho các đối tượng
            generateID();
            generateOrderID();
            // Lấy thông tin nhà sản xuất
            getManufacturer();
            // Bắt đầu tính giờ với timer
            timer1.Start();
        }
        ...
        public void CLear() // xóa nội dung các trường nhập và trả lại giá trị mặc định cho combobox
        {
          //  txtIDCode.Text = "";
            txtName.Text = "";
            txtPrice.Text = "";
        ...
        public void generateID()
        {
            ...
            // Tạo một chuỗi kết quả bằng cách lặp lại chuỗi 'chars' 5 lần, chọn ngẫu nhiên một ký tự từ 'chars'
            // mỗi lần lặp và sau đó gộp các ký tự được chọn thành một chuỗi mới
            var result = new string(
                Enumerable.Repeat(chars, 5) // Lặp lại chuỗi 'chars' 5 lần
                          .Select(s => s[random.Next(s.Length)]) // Chọn một ký tự ngẫu nhiên từ 'chars' mỗi lần lặp
                          .ToArray()); // Chuyển đổi kết quả thành một mảng ký tự và sau đó thành một chuỗi
        ...
        {
            lblTempID.Text = listView1.FocusedItem.Text; // cập nhật ID
            lblTempName.Text = listView1.FocusedItem.SubItems[2].Text;   // cập nhật tên          
        }
        ...
        public void getData()
        {
            //hiển thị data từ Database đến lstview
            try
            {
        ...
        {
            // Nếu các trường không được nhập đầy đủ
            ...
            else 
            {
                try
                {
                    //chèn dữ liệu mới vào bảng tblneworder
        ...
        public void InsertTrail()
        {
            try
            {
                // Câu lệnh SQL để chèn dữ liệu vào bảng tblAuditTrail
                string sql = @"INSERT INTO tblAuditTrail VALUES(@Dater,@Transactype,@Description,@Authority)";
                cm = new SqlCommand(sql, cn);

                // Thêm các giá trị tham số vào đối tượng SqlCommand
                cm.Parameters.AddWithValue("@Dater", lblDate.Text); // Ngày thực hiện
                cm.Parameters.AddWithValue("@Transactype", "Insertion"); // Loại giao dịch
                cm.Parameters.AddWithValue("@Description", "Order:" + txtOrderID.Text + " has been sent to orders!"); // Mô tả
                cm.Parameters.AddWithValue("@Authority", "Admin"); // Người thực hiện
            ...
            {
                // Hiển thị thông báo lỗi khi gặp lỗi SQL
